...
Proof.
  intros. destruct K.
  + simpl. eapply aE_RedCong. apply H. apply R_Refl.
  + simpl. eapply aE_RedCong. (* apply R_CongLet with (x:="x") (A:=Bool).
...
Proof.
  destruct K.
  + simpl. eapply aE_RedCong; apply R_Refl.
  + simpl.
    assert (Equiv g (eLet (bind n m) B) (bind (bind n m) B)).
    * eapply aE_RedCong. apply R_RedR. apply R_Let. apply R_Refl.
...
Proof.
  destruct K.
  + unfold unrestrict_cont. simpl. eapply aE_RedCong; apply R_Refl.
  + unfold unrestrict_cont. simpl. eapply aE_RedCong.
